...
public class HeapPage implements Page {
    ...
    */
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (recordId == null || !recordId.getPageId().equals(pid)) throw new DbException("the tuple to be deleted not residing in the target page.");
        t.setRecordId(null);

        if (!getSlot(recordId.tupleno())) throw new DbException("the tuple has already been removed in the target page.");
        setSlot(recordId.tupleno(), false);
    ...
    public void addTuple(Tuple t) throws DbException {
        if (getNumEmptySlots() == 0 || !t.getTupleDesc().equals(td)) throw new DbException("tuple add error occur in the specific page.");
        int slotNo = 0;
        while (getSlot(slotNo)) slotNo += 1;
    ...
    public int getNumEmptySlots() {
        int acc = 0;
        for (int i = 0; i < tuples.length; i += 1) {
            if (!getSlot(i)) acc += 1;
        }
        return acc;
    }
    ...
    public boolean getSlot(int i) {
        byte byteInfo = header[i/8];
        int mask = (1 << (i % 8));
        return (byteInfo & mask) != 0;
    }
    ...
    private void setSlot(int i, boolean value) {
        ...
        int operand = 1 << (i % 8);
        if (value) {
            header[headerIdx] = (byte) (header[headerIdx] | operand);
        } else {
            header[headerIdx] = (byte) (header[headerIdx] - operand);
        }
    }
    ...
    private class TupleIterator implements Iterator<Tuple> {
        ...
        public boolean hasNext() {
            while (i < tuples.length && tuples[i] == null) i += 1;
            if (i < tuples.length) return true;
            return false;
        }
