...
public class JoinOptimizer {
    ...
    */
    ...
    public Vector<LogicalJoinNode> orderJoins(HashMap<String, TableStats> stats, 
                                              ...
                                              boolean explain) throws ParsingException, NoSuchFieldException {
        ...
        for (int i = 1; i <= joins.size(); i += 1) {
            for (Set<LogicalJoinNode> set : enumerateSubsets(joins, i)) {
                ...
                CostCard cc = null;
                for (LogicalJoinNode to_remove : set) {
                    CostCard candidate_cc = computeCostAndCardOfSubplan(stats, filterSelectivities, to_remove, set, lowestCost, pc);
                    if (candidate_cc != null) {
                        lowestCost = candidate_cc.cost;
                        cc = candidate_cc;
                    }
                }
                if (lowestCost < Double.POSITIVE_INFINITY) {
                    pc.addPlan(set, cc.cost, cc.card, cc.plan);
                }
            }
        }

        Vector<LogicalJoinNode> res = pc.getOrder(enumerateSubsets(joins, joins.size()).iterator().next());
        if (explain) {
...
@SuppressWarnings("unchecked")
    private CostCard computeCostAndCardOfSubplan(HashMap<String, TableStats> stats, 
                                                ...
                                                PlanCache pc) throws ParsingException, NoSuchFieldException {
    ...
    private boolean isPkey(String table, String field) throws NoSuchFieldException {
    ...
    private boolean hasPkey(Vector<LogicalJoinNode> joinlist) throws NoSuchFieldException {
        for (LogicalJoinNode j: joinlist) {
